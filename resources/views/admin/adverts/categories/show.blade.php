            ...
            <table class="table mb-0" style="font-size:0.9rem;">
                <tbody>
                    <tr style="background:#f8f9fb;">
                        ...
                        <td style="color:#1a1a2e;border:none;padding:12px 16px;">{{ $category->id }}</td>
                    </tr>
                    <tr>
                        <td style="font-weight:600;color:#6b7280;border:none;padding:12px 16px;">Name</td>
                        <td style="color:#1a1a2e;border:none;padding:12px 16px;">{{ $category->name }}</td>
                    </tr>
                    <tr style="background:#f8f9fb;">
                        <td style="font-weight:600;color:#6b7280;border:none;padding:12px 16px;">Slug</td>
                        <td style="border:none;padding:12px 16px;font-family:monospace;font-size:0.85rem;">{{ $category->slug }}</td>
                    </tr>
                    <tr>
                    ...
                    <tr style="background:#f8f9fb;">
                        <td style="font-weight:600;color:#6b7280;border:none;padding:12px 16px;">Depth</td>
                        <td style="color:#1a1a2e;border:none;padding:12px 16px;">{{ $category->depth ?? 0 }}</td>
                    </tr>
                </tbody>
            </table>
        </div>
    ...
    <div class="card">
        <div class="card-body" style="padding:1.25rem 1.75rem 0.75rem;">
            <h6 class="fw-semibold mb-3" style="color:#1a1a2e;font-size:0.9rem;">
                Ichki kategoriyalar
                <span style="background:#f0f1f5;color:#6b7280;padding:2px 8px;border-radius:20px;font-size:0.75rem;font-weight:400;margin-left:6px;">
                    {{ $children->count() }}
                </span>
            </h6>
        </div>
            ...
            <table class="table table-hover mb-0" style="font-size:0.9rem;">
                <tbody>
                    ...
                    <tr style="border-bottom:1px solid #f0f1f5;">
                        ...
                        <td style="padding:10px 16px;color:#6b7280;font-family:monospace;font-size:0.82rem;">
                            {{ $child->slug }}
                        </td>
                        <td style="padding:8px 16px;width:80px;">
                            <div class="d-flex gap-1 justify-content-end">
                                ...
                                <form method="POST"
                                      action="{{ route('admin.adverts.categories.destroy', $child) }}"
                                      onsubmit="return confirm('O\'chirishni tasdiqlaysizmi?')">
                                    @csrf @method('DELETE')
                                    <button type="submit"
